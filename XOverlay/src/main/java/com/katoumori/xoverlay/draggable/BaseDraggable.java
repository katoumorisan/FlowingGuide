package com.katoumori.xoverlay.draggable;
...
public abstract class BaseDraggable implements View.OnTouchListener {
     ...
     * Toast 显示后回调这个类
     */
    public void start(XOverlay<?> overlay) {
        mOverlay = overlay;
        mDecorView = overlay.getDecorView();
        mWindowManager = overlay.getWindowManager();
        mWindowParams = overlay.getWindowParams();

        mDecorView.setOnTouchListener(this);
    }

    protected XOverlay<?> getXToast() {
        return mOverlay;
    }
